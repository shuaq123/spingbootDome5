package com.example.demo4.config.ImageUtil;
...
public class ImageUtil {
        ...
        for (int i = 0; i < targetWidth; i++) {
            for (int j = 0; j < targetHeight; j++) {
                ...
                if (rgb == 1) {
                    ...
                    oriImage.setRGB(x + i, y + j, 0x00000000);
                } else {
                    //这里把背景设为透明
                    targetImage.setRGB(i, j, rgb_ori & 0x00000000);
                }
            }
        }
    }
    ...
    public static Map<String, Object> createImage (File file, Map < String, Object > resultMap){
        try {
            ...
            String bb =getImageBASE64(oriImage);
            resultMap.put("smallImage", getImageBASE64(targetImage));//小图
            String aa=getImageBASE64(targetImage);
    ...
    public static Map<String, Object> createImage (String imgUrl, Map < String, Object > resultMap){
        try {
            ...
            System.out.println(widthRandom);
            System.out.println(widthRandom);
            resultMap.put("xWidth", widthRandom);
            resultMap.put("yHeight", String.valueOf(heightRandom)+"px");
        } catch (Exception e) {
            //logger.info("创建图形验证码异常", e);
        } finally {
            return resultMap;
    ...
    public static String getImageBASE64 (BufferedImage image) throws IOException {
        ...
        String BASE64IMAGE = new String(Base64.getEncoder().encode(imagedata), StandardCharsets.UTF_8).toString();
